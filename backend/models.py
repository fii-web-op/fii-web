...
class PageView(Base):
    """One row per public page load — drives the admin analytics dashboard.

    Session id is generated on the client (per browser tab via sessionStorage)
    so we can count unique visitors without storing any PII. Admin previews
    are skipped on the client.
    """

    __tablename__ = "page_views"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    page_key:   Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
